...
from models import PropensityDropout
import random
...
if __name__ == '__main__':
    with tf.Session() as sess:    
        ...
        for i in range(0, 1):
            start = time.time()
            writer = tf.summary.FileWriter('logs2/counterfactual_models/dataset'+str(i) +'/',sess.graph)
            last_batch = 0
            network = PropensityDropout()
            for epoch in range(2):
                ...
                f = open('time_for_propensity_drop')
