...
from models import PropensityDropout, PropensityDropout2, AutoEncoder
import random
...
if __name__ == '__main__':
    with tf.Session() as sess:    
        ...
        for i in range(0, 1):
            writer = tf.summary.FileWriter('logs_learning_representations/autoencoder/dataset'+str(i) +'/',sess.graph)
            last_batch = 0
            ...
            for epoch in range(config.train_epochs):
                ...
                network.treated_model.save_weights(config.autoencoder_model_dir+str(i) + '/' + 'treated' +'/')
                network.control_model.save_weights(config.autoencoder_model_dir+str(i) + '/' + 'control' +'/')
